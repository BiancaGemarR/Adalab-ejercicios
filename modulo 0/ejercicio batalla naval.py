class ship:
    ...
    def place_ship(self, board, start_row, start_col, direction):
        positions = []
        if direction =='H':
            if start_col + self.size > len(board[0]): return False
            for i in range(self.size):
                if board[start_row][start_col + i] != ' ': return False
            positions.append((start_row, start_col + i))
        elif direction == 'V':
            if start_row + self.size > len(board[0]): return False
            for i in range(self.size):
                if board [start_row + i] [start_col] != '': return False
...
class guerranavalGame:
    def __init__(self):
        self.player1 = player("Jugador 1")
        self.player2 = player("Jugador 2")

    def play(self):
        print("Bienvenido al juego Guerra Naval")
...
#Crear una instancia y empezar el juego
game = guerranavalGame()
game.play()
